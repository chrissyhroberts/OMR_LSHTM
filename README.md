...
### 4. Instructions for making a new survey based on the template
...
Here, Q12 is TRUE. Looking at our decode list, that tells us that the form was filled during week 3.
Q61 is TRUE, meaning that the subject ate little today.
Q273 is TRUE, meaning the subject went to school today. 
Q334 is TRUE, indicating that the subject was not in their period. 

Note also that the way this works is that a field is either TRUE or NA. In downstream software (i.e. R) you'll want to replace NAs with FALSE, probably when using `read_csv`.

You may also need to do some funky stuff with the variables. In the example we have four possible answers to the single 'have you eaten today?' question, but the responses are stored in four separate fields. You'll need to mutate the data set to get a new variable that summarises which of the four answers are given. In R this would involve

* binding the variable names on to the generic question IDs
* using `case_when` to mutate the data appropriately

The overall approach we've taken puts very little emphasis on fiddling around with the JSON files for the OMR stage and instead requires you to perform all the munging and data sanity stuff software like R, which are more familiar to researchers.
...
### 6. template.json structure explained
...
### 7. Defining question types inside template.py
...
'vals': range(10)
```
In the generic template example, there's only one kind of question
...
This is a single box, which if checked, returns TRUE. If unchecked, returns NA/NULL
Because we emphasise the value of the 510 field grid template, and reliance on munging data in R, additional question types are somewhat redundant.

### 8. About the generic template
